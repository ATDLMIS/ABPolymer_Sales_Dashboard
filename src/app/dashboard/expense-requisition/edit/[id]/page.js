...
const page = ({params}) => {
  ...
  const getBDExpReqNo = async () => {
    const res = await Axios.post(
      '?action=generate_new_BDExpReq_number'
    );
  ...
  const getPreviosuData = async id => {
    ...
    InstitutionTypeID: res.data.BDExpReq.InstitutionTypeID,
    InstitutionID: res.data.BDExpReq.InstitutionID,
    TotalAmount: res.data.BDExpReq.TotalAmount,
    UserID: res.data.BDExpReq.UserID,
    BDExpReqDetails: res.data.BDExpReqDetails.length ? res.data.BDExpReqDetails.map(item =>{
      return {
        id: item.SL,
...
if(params.id){
  ...
  const getInstutionName = async id => {
    const res = await Axios.get(
      `?action=get_institutionName&InstitutionTypeID=${id}`
    );
    if (res.data.length) {
      setIntName(res.data);
    }
  ...
  const getTeachers = async id => {
    const res = await Axios.get(
      `?action=get_institutionTeacher&InstitutionID=${id}`
    );
    if (res.data.length) {
  ...
  const handleSubmit = async event => {
    ...
    const res = await Axios.put('?action=Update_BDExpReqAll', dataWillbeSubmitted)
...
<td className="whitespace-nowrap px-6 py-4 flex justify-center items-end h-full gap-3">
        ...
        <div className="mt-5">
            <button className="capitalize bg-primary1 px-5 py-1 text-white rounded-md w-full" type="submit">
              Save Order
            </button>
          </div>
      </form>

...
const page = ({ params }) => {
  ...
  const handleAuthorize = async () => {
    ...
    UserID: formData.UserID
     ...
     const res = await Axios.post('?action=create_sndApprovalDetails', dataWillBeSubmitted)
  ...
  const handleReject = async () => {
   ...
   const res = await Axios.post('?action=create_sndApprovalRejected_Cancelled', dataWillBeSubmitted)
   router.push('/dashboard/sales-order-approval')
  }
...
{returnInfo.data.length > 0 && <div>
  <div className="flex flex-col">
        ...
        <div className="flex justify-between py-3 px-1">
          ...
          <button className="bg-primary1 px-6 py-2 rounded-md text-white" onClick={handleAuthorize}>
            Approve
          </button>
        </div>
      </div>
